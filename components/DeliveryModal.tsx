...
export default function DeliveryModal({
  ...
  useEffect(() => {
    ...
    fetch(`/api/deliveries?reservation=${reservationId}`, { cache: 'no-store' })
      .then((res) => res.json())
      .then((existing) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        throw new Error(data.error || 'Failed to record key delivery handoff');
      }
      ...
      <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-4 my-8">
        <div className="flex items-center justify-between border-b pb-3">
          <div className="flex items-center space-x-2 rtl:space-x-reverse">
            ...
            <h3 className="text-xl font-bold text-slate-900">
              {isEditMode ? 'Update Delivery Handoff' : t('deliveryPopupTitle')}
            </h3>
          </div>
        ...
        {isLoadingExisting ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  placeholder="100"
                  className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500 text-sm font-semibold"
                />
              </div>
            ...
            {/* National ID Photo Upload Section */}
              ...
              <div className="border-2 border-dashed border-slate-200 rounded-xl p-4 text-center hover:border-blue-400 transition-colors">
                ...
                >
                  ...
                  <span className="text-xs font-semibold text-blue-600 hover:underline">
                    {isUploading ? 'Uploading photos...' : t('uploadPhotos')}
                  </span>
                  <span className="text-[10px] text-slate-400">{t('uploadIdInstruction')}</span>
                </label>
              </div>
                ...
                <div className="grid grid-cols-3 gap-2 pt-2">
                  {uploadedPhotos.map((url, idx) => (
                    <div key={idx} className="relative group rounded-lg overflow-hidden border border-slate-200 aspect-video">
                      <img src={url} alt={`ID Card ${idx + 1}`} className="w-full h-full object-cover" />
                      <button
                        type="button"
